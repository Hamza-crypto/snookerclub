        ...
        @foreach ($third_round as $tournament)
            ...
            <div class="bracket    " title="Click to view a list of matches">
                <div class="bracket__participant bracket__participant--home">
                    <span class="bracket__name @if ($tournament['winner'] == $tournament['player_1_id']) bracket__name--advancing @endif">
                        {{ $tournament['player_1'] }}
                    </span>
                </div>
                <div class="bracket__result bracket__result--home">
                    <div class="result "> {{ $tournament['score_player_1'] == 0 ? '' : $tournament['score_player_1'] }}
                    </div>
                </div>
                <div class="bracket__participant bracket__participant--away">
                    <span class="bracket__name @if ($tournament['winner'] == $tournament['player_2_id']) bracket__name--advancing @endif">
                        {{ $tournament['player_2'] }}</span>
                </div>
                <div class="bracket__result bracket__result--away">
                    <div class="result "> {{ $tournament['score_player_2'] == 0 ? '' : $tournament['score_player_2'] }}
                    </div>
                </div>
            </div>
